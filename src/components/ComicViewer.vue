...
async function vote(variant: 'a' | 'b') {
  ...
  try {
    ...
    voted.value = variant;
    votedModel.value = result.selected?.model || comic.value?.variants[variant].model || null;
    ...
    if (usingFallback.value && comic.value) {
      voted.value = variant;
      votedModel.value = comic.value.variants[variant].model;
...
function votedVariantLabel() {
  return voted.value ? voted.value.toUpperCase() : '';
}
  ...
  <div class="comic-viewer">
    ...
    <div v-else-if="comic" class="comic-display">
      ...
      <div v-if="voted" class="thank-you">
        ...
        <p class="model-reveal">
          You picked Variant <strong>{{ votedVariantLabel() }}</strong>,
          generated by <code>{{ votedModelLabel() }}</code>.
        </p>
